...
for target in target_dirs:
    ...
    for suffix in ["flex", "noflex"]:
        run_name = f"oracle_100_10_{suffix}"
        run_dir = target / "runs" / run_name

        if suffix == "flex":
            flexref_block = f"""[flexref]
...
"""
        ...
        config_path = configs_dir / f"config_oracle_{suffix}.cfg"
        config_path.write_text(config_content)

    #print(f"Processed {target.name}: TBL and config generated")
