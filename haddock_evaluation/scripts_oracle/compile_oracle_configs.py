#!/usr/bin/env python3
"""Generate oracle HADDOCK3 configurations.

Note: This script requires ti.tbl files to exist. Generate them first using:
    python scripts/true_interface_restraints.py
"""
...
def generate_config(mol1, mol2, complex_pdb):
    """Generate oracle config content."""
    return f"""run_dir = "haddock_oracle_100_10"
molecules = ["{mol1}","{mol2}"]
ncores = 64
...
def main():
    ...
    for unit_dir in sorted(target_dir.iterdir()):
        ...
        config_path = configs_dir / "config_oracle.cfg"
        config_path.write_text(config_content)
        print(f"  Created configs/config_oracle.cfg")
